...
client.on("messageCreate", message => {
    var parolacce = ["porco dio", "cazzo", "diocane", "dio cane","merda", "DIOCANE", "NEGRO", "n3gro", "negr0", "Negro", "Cazzo", "Porco dio", "Dio cane", "Porca madonna", "PORCA MADONNA","porca madonna" ,"gay", "G4y", "GAY", "Cancro","Gay", "c4ncro", "C4ncro"]
    var trovata = false; 
...
client.on("messageReactionAdd", async function (messageReaction, user) {
    ...
    if (messageReaction.message.id == "993202830003425280") {
        if (messageReaction._emoji.name == "👍") {
            var utente = messageReaction.message.guild.members.cache.find(x => x.id == user.id);
            utente.roles.add("926240172272668693");
        }
    }
})
...
client.on("messageReactionRemove", async function (messageReaction, user) {
    ...
    if (messageReaction.message.id == "993202830003425280") {
        if (messageReaction._emoji.name == "👍") {
            var utente = messageReaction.message.guild.members.cache.find(x => x.id == user.id);
            utente.roles.remove("926240172272668693");
        }
    }
})
